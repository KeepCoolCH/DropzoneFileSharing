...
$isAdminPostAction = $_SERVER['REQUEST_METHOD'] === 'POST'
    && array_reduce($adminPostActions, fn($found, $key) => $found || isset($_POST[$key]), false);
if ($isAdminPostAction) {
...
function loadEnvFile(string $path): array {
    ...
    foreach ($lines as $line) {
        ...
        $data[dz_clean_line_value($key)] = dz_clean_line_value($val);
    }
    return $data;
}
...
function saveEnvFile(string $path, array $data): void {
    ...
    foreach ($data as $k => $v) {
        $lines[] = dz_clean_line_value((string)$k) . '=' . dz_clean_line_value((string)$v);
    }
...
if (isset($_POST['env_update'])) {
    $envData['SMTP_HOST'] = dz_clean_line_value($_POST['SMTP_HOST'] ?? '');
    $envData['SMTP_PORT'] = dz_clean_line_value($_POST['SMTP_PORT'] ?? '');
    $envData['SMTP_USER'] = dz_clean_line_value($_POST['SMTP_USER'] ?? '');
    $envData['SMTP_PASS'] = dz_clean_line_value($_POST['SMTP_PASS'] ?? '');
    $envData['SMTP_FROM_ADDRESS']  = dz_clean_line_value($_POST['SMTP_FROM_ADDRESS'] ?? '');
...
<body class="admin">
    ...
    <div id="form">
        ...
        <table>
            ...
            <tr>
                <td data-label="<?= $t['th_file'] ?>"><strong><?=htmlspecialchars($name)?></strong><br><br><?=htmlspecialchars($e['link'])?><br><br><?= $t['uploaded_by'] ?>: <i><?=htmlspecialchars((string)$upl_user)?></i></td>
