...
$info = $fileData[$token];
$filePath = dz_uploads_file_path((string)($info['path'] ?? ''));
if ($filePath === null) {
...
if (!empty($info['password'])) {
    ...
    if (!$pwValid) {
        ...
        $fileName = htmlspecialchars(basename($info['name'] ?? $filePath), ENT_QUOTES, 'UTF-8');
...
header('Content-Disposition: attachment; filename="' . dz_header_filename((string)($info['name'] ?? basename($filePath))) . '"');
