...
function updateSelectedFile() {
    if (fileInput.files.length > 0) {
        const paths = Array.from(fileInput.files).map(f => f.webkitRelativePath || f.name);
        const tree = buildTree(paths);
        selectedFileDiv.innerHTML =
            (paths.length > 1 ? t.selected_files_plural : t.selected_files) +
            ":" + renderTree(tree);
...
function renderTree(tree) {
	...
	const keys = Object.keys(tree).sort((a, b) => a.localeCompare(b, 'de', { numeric: true }));
  	...
  	const folders = keys.filter(k => tree[k] !== null);
  	const files   = keys.filter(k => tree[k] === null);
