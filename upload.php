...
function dz_get_base_url(): string
{
    ...
    if ($forwardedPrefix !== '') {
        $basePath = '/' . trim(str_replace('\\', '/', $forwardedPrefix), '/');
        if (!preg_match('#^/[A-Za-z0-9._~!$&\'()*+,;=:@/%-]*$#', $basePath)) {
            $basePath = '';
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $directories = [];
    $rawDirectories = $_POST['directories'] ?? '[]';
    $decodedDirectories = json_decode((string)$rawDirectories, true);
    if (is_array($decodedDirectories)) {
        foreach ($decodedDirectories as $dirPath) {
            if (!is_string($dirPath)) {
                continue;
            }
            $dirPath = trim(str_replace('\\', '/', $dirPath), '/');
            if ($dirPath === '' || strlen($dirPath) > 4096 || !preg_match('//u', $dirPath) || preg_match('/[\x00-\x1F\x7F]/', $dirPath)) {
                continue;
            }
    ...
    $ensure_space_for_total = function (string $totalSize) use ($bi_add, $bi_cmp): void {
        global $uploadDir;
        ...
        $required = $bi_add($bi_add($totalSize, $totalSize), (string)(10 * 1024 * 1024));
        if ($bi_cmp($required, (string)$free) > 0) {
            http_response_code(507);
            header('Content-Type: text/plain; charset=UTF-8');
            exit('ERR insufficient disk space');
        }
    };

    $ensure_space_for_total($bi_norm((string)$totalSizeIn));
    ...
    if ($action === 'finalize') {
        ...
        foreach (array_keys($directories) as $dirPath) {
            $dirFullPath = $stagingDir . '/' . $dirPath;
            if (!is_dir($dirFullPath) && !mkdir($dirFullPath, 0777, true)) {
                $failFinalize(500, 'cannot create staged directory');
            }
        }

        foreach (array_keys($directories) as $dirPath) {
            @unlink($stagingDir . '/' . $dirPath . '/.dropzone-empty-folder-placeholder');
        }
        ...
        if (Config::$default['send_email'] && $mailChoice === 'yes') {
            ...
            $verifyUrl = $baseUrl . "/verify.php?lang=" . rawurlencode($lang) . "&email=$encEmail&token=$encToken";
        ...
        if (!Config::$default['only_upload']) {
            if (!Config::$default['send_email']) {
                ...
                echo $t['your_link'] . " 
                <a id='link' href='$safeLink' target='_blank'>$safeLink</a><br><br>
