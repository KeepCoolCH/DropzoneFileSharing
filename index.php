...
$footer = '<footer>' . $t['title'] . ' V.1.0 © 2025 by Kevin Tobler - <a href="https://kevintobler.ch" target="_blank">www.kevintobler.ch</a></footer>';
...
if (isset($_GET['t'])) {
	...
	if (!empty($info['password'])) {
		if ($_SERVER['REQUEST_METHOD'] !== 'POST' || $_POST['pw_token'] !== $token || !isset($_POST['pw_input']) || !password_verify($_POST['pw_input'], $info['password'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    if ($fileCount === 0) {
        ...
        if ($fileCount === 1) {
            ...
            if (!mkdir($tmpDir) && !is_dir($tmpDir)) {
                ...
                for ($i = 0; $i < $fileCount; $i++) {
                    ...
                    if (is_uploaded_file($tmp)) {
                        $safeName = preg_replace('/[^a-zA-Z0-9._\- äöüÄÖÜß]/u', '_', $name);
                        $tmpTarget = $tmpDir . '/' . $safeName;
                        if (!move_uploaded_file($tmp, $tmpTarget)) {
                            $message = $t['error_upload_file'] . $name . ".";
                            break;
                        }
                        // Store path relative to uploads folder
                        $relativePath = $token . '/' . $safeName;
                        $targetFiles[] = ['path' => $relativePath, 'name' => $name];
                    }
                }
                if ($message === '') {
                    $zipName = "$token.zip";
        ...
        if ($message === '' && count($targetFiles) === 1) {
            ...
            file_put_contents($dataFile, json_encode($fileData));
            $link = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . "?lang=$lang&t=$token";
            $message = $t['your_link'] . " 
