...
<body>
	...
	document.getElementById('mailChoice').addEventListener('change', function() {
		updateUI(this.value);
	});
	
	document.addEventListener('DOMContentLoaded', function () {
		updateUI(document.getElementById('mailChoice').value);
	});
	...
	function disableUploadButtons() {
		...
		if (btnNormal) {
			...
			btnNormal.textContent = '<?= $t['uploading'] ?>';
			btnNormal.style.backgroundColor = '#ccc';
			btnNormal.style.color = '#666';
			btnNormal.style.cursor = 'not-allowed';
		}
		if (btnEmail) {
			btnEmail.disabled = true;
			btnEmail.textContent = '<?= $t['uploading'] ?>';
			btnEmail.style.backgroundColor = '#ccc';
			btnEmail.style.color = '#666';
